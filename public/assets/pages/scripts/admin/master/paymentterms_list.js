$('#datatable_list').DataTable({
    ...
    "ajax": assets + "/" + panel_text + "/paymentterms/data",
	"columns": [
    ...
    var handleValidation = function () {
        ...
        form.validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    type: "POST",
                    url: assets  + "/" +  panel_text + '/paymentterms/data',
                    data: formData,
                    contentType: false,
                    cache: false,
                    processData: false,
...
function add_edit_modal(id, mode) {
    ...
    if (mode == "Add") {
        ...
        $('#paymentterms').val('');
        $('#due_type_value').val('');
        $('#due_type').prop('checked', true).trigger('change');
        $('#due_type').val('1');
        $('#due_type').prop( "checked", true );
        ...
        $('#id').val(id);
        $('#paymentterms').val($("#gname" + id).html());
        $('#due_type_value').val($("#gvalue" + id).html());
        var status = $("#gstatus" + id).html();
        var type = $("#gtype" + id).html();
        ...
        if(type == 1){
            ...
            $(".bootstrap-switch-id-status").addClass('bootstrap-switch-off');
            $('#due_type').prop('checked', false).trigger('change');
        }
        $('#add_modal_box').trigger('click');
    }
}
function clearfieldvalue() {
    ...
    $('#paymentterms').closest('.form-group').removeClass('has-error');
}
