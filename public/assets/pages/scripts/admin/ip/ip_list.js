$('#datatable_list').DataTable({
    ...
    "ajax": assets + "/" + panel_text + "/ip/data",
	"columns": [
...
function add_edit_modal(id, mode) {
    ...
    if (mode == "Add") {
        ...
        $('#id').val(id);
        $('#description').html($("#gidescription" + id).html());
        $('#ip').val($("#gip" + id).html());
        var status = $("#gstatus" + id).html();
        if(status == 1){
            $('.switchBootstrap').parent().parent().addClass('bootstrap-switch-on');
        }else{
            $(".bootstrap-switch-id-status").parent().parent().addClass('bootstrap-switch-off');
        }
        $('#add_modal_box').trigger('click');
    }
}
