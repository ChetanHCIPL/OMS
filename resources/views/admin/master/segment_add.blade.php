...
<div class="content-wrapper">
  <div class="content-body">
	...
	<section class="content-header clearfix">
		<h3>Segment <strong><span class="text-muted accent-3">{{((isset($data[0]['name'])?' - '.reduceTitleName($data[0]['name']):''))}}</span></strong></h3>
		<ol class="breadcrumb">
              ...
              <div class="card-head ">
                <div class="card-header">
                  <div class="float-right">
                    ...
                    <a href="{{route('segment',['mode' => 'view', 'id' => isset($data[0]['id'])?gen_generate_encoded_str($data[0]['id'], '3', '3', ''):''])}}" title="View"><span class="btn btn-icon btn-secondary btn-light waves-effect waves-light"><i class="la la-eye"></i></span></a>@endif
                  </div>
                </div>
              </div>
            @endif
                              ...
                              <div class="row">
                                <div class="form-body"> 
                                    <div class="form-group row mx-auto">
                                        <label class="col-md-3 label-control">Segment Name <span class="required">*</span></label>
                                        <div class="col-md-9">
                                    ...
                                    <div class="form-group row mx-auto d-none">
                                        <label class="col-md-3 label-control">Medium <span class="required">*</span></label>
                                        <div class="col-md-9">
                                            <select class="selectize-select" id="country_id" name="medium_id" placeholder="Select Medium">
                                                <option value="">Select Medium</option>
                                                @if(!empty($mediums))
                                                    @foreach($mediums as $medium)
                                                        @if(isset($data[0]['medium_id']) && $data[0]['medium_id'] == $medium['id'])
                                    ...
                                    <div class="form-group row mx-auto d-none">
                                        <label class="col-md-3 label-control">Semester <span class="required">*</span></label>
                                        <div class="col-md-9">
                                            <select class="selectize-select" id="country_id" name="semester_id" placeholder="Select Semester">
                                                <option value="">Select Semester</option>
                                                @if(!empty($semesters))
                                                    @foreach($semesters as $semester)
                                                        @if(isset($data[0]['semester_id']) && $data[0]['semester_id'] == $semester['id'])
                                                            @php $selected = "selected"; @endphp
                                                        @else
                                                            @php $selected = ""; @endphp
                                                     s   @endif
...
<script src="{{ asset('/assets/pages/scripts/admin/master/segment_add.js')}}" type="text/javascript"></script>
