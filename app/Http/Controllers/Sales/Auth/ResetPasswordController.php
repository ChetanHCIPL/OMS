...
class ResetPasswordController extends Controller
{
    ...
    */
    ...
    public function showResetForm(Request $request, $token = null) {
        ...
        return view('sales.auth.passwords.reset')
    ...
    public function reset(Request $request){
        ...
        if(!empty($data)){
            ...
            if(isset($update)){
                return redirect()->route('sales.login')->with('status', trans('passwords.reset')); 
            } 
            else {
                return Redirect::back()->withInput()->withErrors(['status' => trans('passwords.token')]);
            }            
        }
        ## Sales email is not exist in password_resets table or token expired
