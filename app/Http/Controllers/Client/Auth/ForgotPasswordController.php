...
class ForgotPasswordController extends Controller
{
    ...
    */
    ...
    public function sendResetLinkEmail(Request $request){
        ...
        $data = Clients::checkClientEmailExist($vEmail);
        if(!empty($data)){
            ...
            if($insert == 1){
                ...
                ## Send Mail
                $client_name = isset($data[0]['client_name'])?$data[0]['client_name']:"";
                $mail = $mailData = array();
                $mail['to_email'] = $vEmail;
                $mailData['token']  = $token;
                $mailData['name']   = $client_name;
                $mailData['site_logo']   = asset('/images/').'/logo/logo.png';
               
                $sentMail = Mail::send('client/email/reset_password', $mailData, function ($message) use($mail) {
