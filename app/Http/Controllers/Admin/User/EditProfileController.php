...
class EditProfileController extends Controller {
    ...
    public function editProfile() {
       ...
       if($image != '' )
        ...
        return view('admin/user/edit_profile')->with(['data' => $data, 'admin_id' => $admin_id, 'checkImgArr' => $checkImgArr]);
    }
    ...
    private function updateRecord($data = array(), $files = array()) {
        ...
        if (isset($update)) {
            ...
            $log_text = "User " . $data['username'] . " edited";

            if(Auth::guard('admin')->check()) {

                $admin_id = Auth::guard('admin')->user()->id;
            }
            else if(Auth::guard('sales_user')->check()) {

                $admin_id = Auth::guard('sales_user')->user()->id;
            }
            $activity_arr = array("admin_id" => $admin_id, "module_id" => $module_id, "reference_id" => $reference_id, "no" => '', "log_text" => $log_text, "req_data" => $data);
