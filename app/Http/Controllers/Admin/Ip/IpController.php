...
class IpController extends Controller {
    ...
    */
    public function index() {
        return view('admin/ip/ip_list');
    }

    /**
    * Get IP data and pass json response to data table
    *
    * @param  array $request
    * @return json $records
    */
    public function ajaxData(Request $request) {
        ...
        $tot_records_data = Ip::getIpData($iDisplayLength = NULL, $iDisplayStart = NULL, $sort = NULL, $sortdir = NULL, $search, $search_arr)->toArray();
        ...
        $data = Ip::getIpData($iDisplayLength, $iDisplayStart, $sort, $sortdir, $search, $search_arr)->toArray();
        $cnt = count($data);
        for ($i = 0; $i < $cnt; $i++) {

            if($data[$i]->status == Ip::ACTIVE){
                $status =  "Active" ;
            }else if($data[$i]->status == Ip::INACTIVE){
                $status =  "Inactive" ;
            } 
            ...
            $edit = '---';
            $view = '';
            if (per_hasModuleAccess('Ip', 'Edit')) {
            ...
            // if (per_hasModuleAccess('Ip', 'View')) {
            //     $view = '<a href="javascript:void(0)" onclick="view_modal('.$data[$i]->id.',\'View\')" title="View"><span class="btn btn-icon btn-secondary btn-light waves-effect waves-light"><i class="la la-eye"></i></span></span></a>';
            // }

            $records["data"][] = array(
                '<input type="checkbox" name="row_' . $i . '" value="' . $data[$i]->id . '" >',
                '<span id="gdescription' . $data[$i]->id . '"><span class="d-none" id="gidescription' . $data[$i]->id . '">'.$data[$i]->description.'</span>' . $data[$i]->description . '</span>',
                '<span id="gfip' . $data[$i]->id . '"> <span id="gip' . $data[$i]->id . '" class="d-none">' . $data[$i]->ip . '</span>'.$data[$i]->ip  . '</span>',
    ...
    */
    public function PostAjaxData(Request $request) {
        ...
        if (isset($post['customActionName'])) {
            ...
            $module_id = Config::get('constants.module_id.ip');
            if ($post['customActionName'] == "Active" || $post['customActionName'] == 'Inactive') {
                ...
                if(is_array($post['id']) && !empty($post['id'])){
                    foreach($post['id'] as $id){
                        $log_text = "Ip " . $id . " Status updated to ".$post['customActionName'];
                    ...
                    $log_text = "Ip" . $post['id'] . " Status updated to ".$post['customActionName'];
    ...
    */
    private function validateAdd($data = array()) {
        $rules = array(
            'ip' => 'required|unique:mas_ip,ip,NULL,id',
            'description' => 'required',
        );
        $messages = array(
            'ip.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Ip'),
            'ip.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'Ip'),
            'description.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Description'),
        );
    ...
    */
    private function validateEdit($data = array()) {
        $rules = array(
            'ip' => 'required|unique:mas_ip,ip,' . $data['id'] . ',id',
            'description' => 'required',
        );
        $messages = array(
            'ip.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Ip'),
    ...
    */            
    private function insertRecord($data = array()) {
        $insert_array = array(
            ...
            'status' => (isset($data['status']) ? $data['status'] : IP::INACTIVE),
        );

        $insert = Ip::addIp($insert_array);
        if (!empty($insert)) {
            $result = 1;
    ...
    */  
    private function updateRecord($data = array()) {
        ...
        $update = Ip::updateIp($id,$update_array);
    ...
    */ 
    private function updateStatus($id = array(), $status) {
        if (!empty($id)) {
            ...
            $update = Ip::updateIpStatus($id, $status);
        }
        if (isset($update)) {
            return 1;
        } else {
     ...
     /** 
    * Function : Delete Records of  Ip
    * @param  array $id
    * @return boolean
    */
    private function deleteRecord($id = array()) {
        if (!empty($id)) {
            $ipdata = Ip::getIpDataFromId($id);
            if(!empty($ipdata)){
                foreach ($ipdata as $data) {
                    ## Start Add Activity
                    $reference_id = (isset($data['id']) ? $data['id'] : "");
                    $module_id = Config::get('constants.module_id.ip');
                    $log_text = "IP ". $data['ip']. " deleted";
                    $activity_arr= array("admin_id" => Auth::guard('admin')->user()->id, "module_id" => $module_id, "reference_id" => $reference_id, "no" => '', "log_text" => $log_text, "req_data" => $data);
                    usr_addActivityLog($activity_arr);
                    ## End Add Activity
                 }                  
            $delete_var = Ip::deleteIpData($id);
            }
