...
class SMSTemplateController extends Controller { 
    ...
    **/
	public function index(){
		...
		return view('admin.tools.sms_template_list')->with(["section_arr" => $section_arr]);
	}


	/**
     * Function : Get sms data 
     * @param array $request 
     *
    ...
    **/
    public function ajaxData(Request $request) {
        ...
        $tot_records_data = SmsTemplate::getSmsTemplateData($iDisplayLength = NULL, $iDisplayStart = NULL, $sort = NULL, $sortdir = NULL, $search, $search_arr)->toArray();
        ...
        $data = SmsTemplate::getSmsTemplateData($iDisplayLength, $iDisplayStart, $sort, $sortdir, $search, $search_arr)->toArray();
        $cnt = count($data);
        
        for ($i = 0; $i < $cnt; $i++) {

            $section = Config::get('constants.sms_template_section.' . $data[$i]->section);
            ...
            if (per_hasModuleAccess('SMSTemplate', 'Edit')) {
                $edit = ' <a href="' . route('sms-template',['mode' => 'edit', 'id' => $encoded_id]) . '" title="Edit"><span class="btn btn-icon btn-secondary waves-effect waves-light"><i class="la la-edit"></i></span></a>';
            }
            if (per_hasModuleAccess('SMSTemplate', 'View')) {
                $view = '<a href="' . route('sms-template',['mode' => 'view',  'id' => $encoded_id]) . '" title="View"><span class="btn btn-icon btn-secondary btn-light waves-effect waves-light"><i class="la la-eye"></i></span></a>';
            }
    ...
    **/
    private function updateStatus($id = array(), $status) {
        if (!empty($id)) {
            ...
            $update = SmsTemplate::updateSmsTemplateStatus($id, $status);
        }
        if (isset($update)) {
            return 1;
        } else {
    ...
    /**
     * Function : Load Edit/View Sms template
     * @param string mode
     * @param int id
     *
     * @return view
    **/
    public function add($mode = NULL,$id = NULL) {
        $sms_lang_arr = $language= $data = array();

        if(isset($mode) && $mode != ""){
            if (isset($id) && $id != "") {
                $smsId = substr($id, 3, -3);
                $data = SmsTemplate::getSmsTemplate($smsId);
                if(!empty($data) && !empty($data[0]['sms_language'])){
                    foreach($data[0]['sms_language'] as $rowLanguage){
                        $sms_lang_arr[$rowLanguage['lang_code']]['content'] = (isset($rowLanguage['content'])?$rowLanguage['content']:"");
                    }
                }
                if($mode == 'edit'){
                    $mode = 'Update';
                    return view('admin/tools/sms_template_add')->with(['mode' => $mode, 'id' => $smsId, 'data' => $data, 'sms_lang_arr' => $sms_lang_arr]);
                }elseif($mode == 'view'){
                    $mode = 'View';
                    return view('admin/tools/sms_template_view')->with(['mode' => $mode, 'id' => $smsId, 'data' => $data, 'sms_lang_arr' => $sms_lang_arr]);
                }
            }
        }
        abort(404);
    ...
    **/
    private function updateRecord($data = array()) {
        ...
        $update = SMSTemplate::updateSMSTemplate($id, $update_array);
        if (isset($update)) {            
                $result_arr[0]['isError'] = 0;
                ...
                ## Start Add Activity
                $reference_id = $id;
                $module_id = Config::get('constants.module_id.sms_template');
                $log_text = "SMS Template " . $data['type'] . " -  " .$data['sectionName']  . " edited";
