...
use App\Models\BindingJobVendor;

class BindingJobVendorController extends Controller {
    ...
    /**
    *  Load Binding Job Vendors List Page
    *
    * @param    void
    * @return   template
    */
    public function index() {
        return view('admin/master/binding_job_vendor_list');
    }

    /**
    * Get Binding JOb Vendors data and pass json response to data table
    *
    * @param  array $request
    * @return json $records
    */
    public function ajaxData(Request $request) {
        ...
        $tot_records_data = BindingJobVendor::getBindingJobVendorData($iDisplayLength = NULL, $iDisplayStart = NULL, $sort = NULL, $sortdir = NULL, $search, $search_arr)->toArray();
        ...
        $data = BindingJobVendor::getBindingJobVendorData($iDisplayLength, $iDisplayStart, $sort, $sortdir, $search, $search_arr)->toArray();
        $cnt = count($data);
        for ($i = 0; $i < $cnt; $i++) {

            if($data[$i]->status == BindingJobVendor::ACTIVE){
                $status =  "Active" ;
            }else if($data[$i]->status == BindingJobVendor::INACTIVE){
                $status =  "Inactive" ;
            } 
            ...
            $records["data"][] = array(
                ...
                '<span id="gfaddress' . $data[$i]->id . '"> <span id="gaddress' . $data[$i]->id . '" class="d-none">' . $data[$i]->address . '</span>'.$data[$i]->address  . '</span>',
    ...
    */
    public function PostAjaxData(Request $request) {
        ...
        if (isset($post['customActionName'])) {
            ...
            $module_id = Config::get('constants.module_id.binding_job_vendor');
            ...
            if ($post['customActionName'] == "Active" || $post['customActionName'] == 'Inactive') {
                ...
                if(is_array($post['id']) && !empty($post['id'])){
                    foreach($post['id'] as $id){
                        $log_text = "Binding Job Vendor " . $id . " Status updated to ".$post['customActionName'];
                    ...
                    $log_text = "Binding Job Vendor " . $post['id'] . " Status updated to ".$post['customActionName'];
    ...
    */
    private function validateAdd($data = array()) {
        $rules = array(
            'name' => 'required|max:150|unique:mas_book_binders,name,NULL,id',
            'address' => 'required|max:300',
        );
        $messages = array(
            'name.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Binding Job Vendor'),
            'name.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'Binding Job Vendor'),
            'name.max' => sprintf(Config::get('messages.validation_msg.maxlength'), '150', 'Binding Job Vendor'),
            'address.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Address'),
            'address.max' => sprintf(Config::get('messages.validation_msg.maxlength'), '300', 'Address'),
        );
    ...
    */
    private function validateEdit($data = array()) {
        $rules = array(
            'name' => 'required|max:150|unique:mas_book_binders,name,' . $data['id'] . ',id',
            'address' => 'required|max:300',
        );
        $messages = array(
            'name.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Binding Job Vendor'),
    ...
    */            
    private function insertRecord($data = array()) {
        ...
        $insert_array = array(
            ...
            'status' => (isset($data['status']) ? $data['status'] : BindingJobVendor::INACTIVE),
        );

        $insert = BindingJobVendor::addBindingJobVendor($insert_array);
        ...
        if (!empty($insert)) {
            ...
            $module_id = Config::get('constants.module_id.binding_job_vendor');
            $log_text = "Binding Job Vendor " . $data['name'] . " added";
    ...
    */  
    private function updateRecord($data = array()) {
        ...
        $update = BindingJobVendor::updateBindingJobVendor($id,$update_array);
       ...
       if (isset($update)) {
            ...
            $module_id = Config::get('constants.module_id.binding_job_vendor');
            $log_text = "Binding Job Vendor " . $data['name'] . " edited";
    ...
    */ 
    private function updateStatus($id = array(), $status) {
        if (!empty($id)) {
            ...
            $update = BindingJobVendor::updateBindingJobVendorStatus($id, $status);
        }
        if (isset($update)) {
            return 1;
        } else {
    ...
    */
    private function deleteRecord($id = array()) {
        if (!empty($id)) {
            $bindingJobVendors = BindingJobVendor::getBindingJobVendorDataFromId($id);
            if(!empty($bindingJobVendors)){
                foreach ($bindingJobVendors as $data) {
                    ## Start Add Activity
                    $reference_id = (isset($data['id']) ? $data['id'] : "");
                    $module_id = Config::get('constants.module_id.binding_job_vendor');
                    $log_text = "Binding Job Vendor ". $data['name']. " deleted";
                    $activity_arr= array("admin_id" => Auth::guard('admin')->user()->id, "module_id" => $module_id, "reference_id" => $reference_id, "no" => '', "log_text" => $log_text, "req_data" => $data);
                    usr_addActivityLog($activity_arr);
                    ## End Add Activity
                 }                  
            $delete_var = BindingJobVendor::deleteBindingJobVendorData($id);
            }
