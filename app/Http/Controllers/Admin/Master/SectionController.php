...
class SectionController extends Controller {
    ...
    /**
    *  Load Section List Page
    *
    * @param    void
    * @return   template
    */
    public function index() {
        return view('admin/master/section_list');
    }

    /**
    * Get Section data and pass json response to data table
    *
    * @param  array $request
    * @return json $records
    */
    public function ajaxData(Request $request) {
        ...
        $tot_records_data = Section::getSectionData($iDisplayLength = NULL, $iDisplayStart = NULL, $sort = NULL, $sortdir = NULL, $search, $search_arr)->toArray();
        ...
        $data = Section::getSectionData($iDisplayLength, $iDisplayStart, $sort, $sortdir, $search, $search_arr)->toArray();
        $cnt = count($data);
        for ($i = 0; $i < $cnt; $i++) {

            if($data[$i]->status == Section::ACTIVE){
                $status =  "Active" ;
            }else if($data[$i]->status == Section::INACTIVE){
                $status =  "Inactive" ;
            } 
    ...
    */
    public function PostAjaxData(Request $request) {
        ...
        if (isset($post['customActionName'])) {
            ...
            $module_id = Config::get('constants.module_id.section');
            ...
            if ($post['customActionName'] == "Active" || $post['customActionName'] == 'Inactive') {
                ...
                if(is_array($post['id']) && !empty($post['id'])){
                    foreach($post['id'] as $id){
                        $log_text = "Section " . $id . " Status updated to ".$post['customActionName'];
                    ...
                    $log_text = "Section " . $post['id'] . " Status updated to ".$post['customActionName'];
    ...
    */
    private function validateAdd($data = array()) {
        $rules = array(
            'name' => 'required|max:150|unique:mas_section,name,NULL,id',
        );
        $messages = array(
            'name.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Section'),
            'name.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'Section'),
            'name.max' => sprintf(Config::get('messages.validation_msg.maxlength'), '150', 'Section'),
        );
    ...
    */
    private function validateEdit($data = array()) {
        $rules = array(
            'name' => 'required|max:150|unique:mas_section,name,' . $data['id'] . ',id',
        );
        $messages = array(
            'name.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Section'),
            'name.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'Section'),
    ...
    /**
    * Function : Insert Record of Section 
    *
    * @param  array $data
    * @return $result
    */            
    private function insertRecord($data = array()) {
        $insert_array = array(
            ...
            'status' => (isset($data['status']) ? $data['status'] : Section::INACTIVE),
        );

        $insert = Section::addSection($insert_array);
        if (!empty($insert)) {
            $result = 1;
            ...
            $module_id = Config::get('constants.module_id.section');
            $log_text = "Section " . $data['name'] . " added";
    ...
    /**
    * Function : Update Record of Section
    *
    * @param  array $data
    * @return $result;
    */  
    private function updateRecord($data = array()) {
        ...
        $update = Section::updateSection($id,$update_array);
       ...
       if (isset($update)) {
            ...
            $module_id = Config::get('constants.module_id.section');
            $log_text = "Section " . $data['name'] . " edited";
     ...
     /**
    *
    * Function : Update status for Section
    * @param  int $id 
    * @param  string status
    * @return boolean 
    */ 
    private function updateStatus($id = array(), $status) {
        if (!empty($id)) {
            ...
            $update = Section::updateSectionStatus($id, $status);
        }
        if (isset($update)) {
            return 1;
        } else {
     ...
     /** 
    * Function : Delete Records of Section
    * @param  array $id
    * @return boolean
    */
    private function deleteRecord($id = array()) {
        if (!empty($id)) {
            $sections = Section::getSectionDataFromId($id);
            if(!empty($sections)){
                foreach ($sections as $data) {
                    ## Start Add Activity
                    $reference_id = (isset($data['id']) ? $data['id'] : "");
                    $module_id = Config::get('constants.module_id.section');
                    $log_text = "Section ". $data['name']. " deleted";
                    $activity_arr= array("admin_id" => Auth::guard('admin')->user()->id, "module_id" => $module_id, "reference_id" => $reference_id, "no" => '', "log_text" => $log_text, "req_data" => $data);
                    usr_addActivityLog($activity_arr);
                    ## End Add Activity
                 }                  
            $delete_var = Section::deleteSectionData($id);
            }
