...
class SeriesController extends Controller {
    ...
    /**
    *  Load Series List Page
    *
    * @param    void
    * @return   template
    */
    public function index() {
        return view('admin/master/series_list');
    }

    /**
    * Get Series data and pass json response to data table
    *
    * @param  array $request
    * @return json $records
    */
    public function ajaxData(Request $request) {
        ...
        $tot_records_data = Series::getSeriesData($iDisplayLength = NULL, $iDisplayStart = NULL, $sort = NULL, $sortdir = NULL, $search, $search_arr)->toArray();
        ...
        $data = Series::getSeriesData($iDisplayLength, $iDisplayStart, $sort, $sortdir, $search, $search_arr)->toArray();
        $cnt = count($data);
        for ($i = 0; $i < $cnt; $i++) {

            if($data[$i]->status == Series::ACTIVE){
                $status =  "Active" ;
            }else if($data[$i]->status == Series::INACTIVE){
                $status =  "Inactive" ;
            } 
    ...
    */
    public function PostAjaxData(Request $request) {
        ...
        if (isset($post['customActionName'])) {
            ...
            $module_id = Config::get('constants.module_id.series');
            ...
            if ($post['customActionName'] == "Active" || $post['customActionName'] == 'Inactive') {
                ...
                if(is_array($post['id']) && !empty($post['id'])){
                    foreach($post['id'] as $id){
                        $log_text = "Series " . $id . " Status updated to ".$post['customActionName'];
                    ...
                    $log_text = "Series " . $post['id'] . " Status updated to ".$post['customActionName'];
    ...
    */
    private function validateAdd($data = array()) {
        $rules = array(
            'name' => 'required|max:150|unique:mas_series,name,NULL,id',
        );
        $messages = array(
            'name.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Series'),
            'name.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'Series'),
            'name.max' => sprintf(Config::get('messages.validation_msg.maxlength'), '150', 'Series'),
        );
    ...
    */
    private function validateEdit($data = array()) {
        $rules = array(
            'name' => 'required|max:150|unique:mas_series,name,' . $data['id'] . ',id',
        );
        $messages = array(
            'name.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Series'),
            'name.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'Series'),
    ...
    /**
    * Function : Insert Record of Series 
    *
    * @param  array $data
    * @return $result
    */            
    private function insertRecord($data = array()) {
        $insert_array = array(
            ...
            'status' => (isset($data['status']) ? $data['status'] : Series::INACTIVE),
        );

        $insert = Series::addSeries($insert_array);
        if (!empty($insert)) {
            $result = 1;
            ...
            $module_id = Config::get('constants.module_id.series');
            $log_text = "Series " . $data['name'] . " added";
    ...
    /**
    * Function : Update Record of Series
    *
    * @param  array $data
    * @return $result;
    */  
    private function updateRecord($data = array()) {
        ...
        $update = Series::updateSeries($id,$update_array);
       ...
       if (isset($update)) {
            ...
            $module_id = Config::get('constants.module_id.series');
            $log_text = "Series " . $data['name'] . " edited";
    ...
    */ 
    private function updateStatus($id = array(), $status) {
        if (!empty($id)) {
            ...
            $update = Series::updateSeriesStatus($id, $status);
        }
        if (isset($update)) {
            return 1;
        } else {
     ...
     /** 
    * Function : Delete Records of Series
    * @param  array $id
    * @return boolean
    */
    private function deleteRecord($id = array()) {
        if (!empty($id)) {
            $series = Series::getSeriesDataFromId($id);
            if(!empty($series)){
                foreach ($series as $data) {
                    ## Start Add Activity
                    $reference_id = (isset($data['id']) ? $data['id'] : "");
                    $module_id = Config::get('constants.module_id.series');
                    $log_text = "Series ". $data['name']. " deleted";
                    $activity_arr= array("admin_id" => Auth::guard('admin')->user()->id, "module_id" => $module_id, "reference_id" => $reference_id, "no" => '', "log_text" => $log_text, "req_data" => $data);
                    usr_addActivityLog($activity_arr);
                    ## End Add Activity
                 }                  
            $delete_var = Series::deleteSeriesData($id);
            }
