...
class PaymentTermsController extends Controller {
    ...
    /**
    *  Load PaymentTerms List Page
    *
    * @param    void
    * @return   template
    */
    public function index() {
        return view('admin/master/paymentterms_list');
    }

    /**
    * Get PaymentTerms data and pass json response to data table
    *
    * @param  array $request
    * @return json $records
    */
    public function ajaxData(Request $request) {
        ...
        $tot_records_data = PaymentTerms::getPaymentTermsData($iDisplayLength = NULL, $iDisplayStart = NULL, $sort = NULL, $sortdir = NULL, $search, $search_arr)->toArray();
        ...
        $data = PaymentTerms::getPaymentTermsData($iDisplayLength, $iDisplayStart, $sort, $sortdir, $search, $search_arr)->toArray();
        $cnt = count($data);
        for ($i = 0; $i < $cnt; $i++) {

            if($data[$i]->status == PaymentTerms::ACTIVE){
                $status =  "Active" ;
            }else if($data[$i]->status == PaymentTerms::INACTIVE){
                $status =  "Inactive" ;
            } 
            if($data[$i]->due_type == 1){
                $due_type =  "Fixed number of days" ;
            }else if($data[$i]->due_type == 2){
                $due_type =  "Certain day of month" ;
            } 
            ...
            $edit = '---';
            $view = '';
            if (per_hasModuleAccess('PaymentTerms', 'Edit')) {
            ...
            $records["data"][] = array(
                ...
                '<span id="gfname' . $data[$i]->id . '"> <span id="gname' . $data[$i]->id . '" class="d-none">' . $data[$i]->term . '</span>'.$data[$i]->term  . '</span>',
                '<span id="gftype' . $data[$i]->id . '"> <span id="gtype' . $data[$i]->id . '" class="d-none">' . $data[$i]->due_type . '</span>'.$due_type  . '</span>',
                '<span id="gfvalue' . $data[$i]->id . '"> <span id="gvalue' . $data[$i]->id . '" class="d-none">' . $data[$i]->due_type_value . '</span>'.$data[$i]->due_type_value  . '</span>',
    ...
    */
    public function PostAjaxData(Request $request) {
        ...
        if (isset($post['customActionName'])) {
            ...
            $module_id = Config::get('constants.module_id.paymentterms');
            ...
            if ($post['customActionName'] == "Active" || $post['customActionName'] == 'Inactive') {
                ...
                if(is_array($post['id']) && !empty($post['id'])){
                    foreach($post['id'] as $id){
                        $log_text = "PaymentTerms " . $id . " Status updated to ".$post['customActionName'];
                    ...
                    $log_text = "PaymentTerms " . $post['id'] . " Status updated to ".$post['customActionName'];
    ...
    */
    private function validateAdd($data = array()) {
        $rules = array(
            'term' => 'required|max:150|unique:mas_payment_terms,term,NULL,id',
            'due_type_value' => 'required|numeric|gt:0|max:3',
        );
        $messages = array(
            'term.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Payment Terms'),
            'term.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'Payment Terms'),
            'due_type_value.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Due Date'),
            'due_type_value.numeric' => sprintf(Config::get('messages.validation_msg.numeric'), 'Due Date'),
            'due_type_value.gt' => sprintf(Config::get('messages.validation_msg.regex'), 'Due Date'),
        );
    ...
    */
    private function validateEdit($data = array()) {
        $rules = array(
            'term' => 'required|max:150|unique:mas_payment_terms,term,' . $data['id'] . ',id',
            'due_type_value' => 'required|numeric|gt:0',
        );
        $messages = array(
            'term.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'PaymentTerms'),
            'term.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'PaymentTerms'),
            'due_type_value.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Due Date'),
            'due_type_value.numeric' => sprintf(Config::get('messages.validation_msg.numeric'), 'Due Date'),
            'due_type_value.gt' => sprintf(Config::get('messages.validation_msg.regex'), 'Due Date'),
        );
    ...
    /**
    * Function : Insert Record of PaymentTerms 
    *
    * @param  array $data
    * @return $result
    */            
    private function insertRecord($data = array()) {
        $insert_array = array(
            ...
            'due_type' => (isset($data['due_type']) ? 1 : 2),
            'due_type_value' => (isset($data['due_type_value']) ? $data['due_type_value'] : ""),
            'status' => (isset($data['status']) ? $data['status'] : PaymentTerms::INACTIVE),
        );
        $insert = PaymentTerms::addPaymentTerms($insert_array);
        if (!empty($insert)) {
            $result = 1;
            ...
            $module_id = Config::get('constants.module_id.paymentterms');
            $log_text = "PaymentTerms " . $data['term'] . " added";
    ...
    /**
    * Function : Update Record of PaymentTerms
    *
    * @param  array $data
    * @return $result;
    */  
    private function updateRecord($data = array()) {
        ...
        $update = PaymentTerms::updatePaymentTerms($id,$update_array);
       ...
       if (isset($update)) {
            ...
            $module_id = Config::get('constants.module_id.paymentterms');
            $log_text = "PaymentTerms " . $data['term'] . " edited";
    ...
    */ 
    private function updateStatus($id = array(), $status) {
        if (!empty($id)) {
            ...
            $update = PaymentTerms::updatePaymentTermsStatus($id, $status);
        }
        if (isset($update)) {
            return 1;
        } else {
     ...
     /** 
    * Function : Delete Records of  PaymentTerms
    * @param  array $id
    * @return boolean
    */
    private function deleteRecord($id = array()) {
        if (!empty($id)) {
            $paymentterms = PaymentTerms::getPaymentTermsDataFromId($id);
            if(!empty($paymentterms)){
                foreach ($paymentterms as $data) {
                    ## Start Add Activity
                    $reference_id = (isset($data['id']) ? $data['id'] : "");
                    $module_id = Config::get('constants.module_id.paymentterms');
                    $log_text = "PaymentTerms ". $data['term']. " deleted";
                    $activity_arr= array("admin_id" => Auth::guard('admin')->user()->id, "module_id" => $module_id, "reference_id" => $reference_id, "no" => '', "log_text" => $log_text, "req_data" => $data);
                    usr_addActivityLog($activity_arr);
                    ## End Add Activity
                 }                  
            $delete_var = PaymentTerms::deletePaymentTermsData($id);
            }
