...
use App\Models\PrintJobVendor;

class PrintJobVendorController extends Controller {
    ...
    /**
    *  Load Print Job Vendors List Page
    *
    * @param    void
    * @return   template
    */
    public function index() {
        return view('admin/master/print_job_vendor_list');
    }

    /**
    * Get Print JOb Vendors data and pass json response to data table
    *
    * @param  array $request
    * @return json $records
    */
    public function ajaxData(Request $request) {
        ...
        $tot_records_data = PrintJobVendor::getPrintJobVendorData($iDisplayLength = NULL, $iDisplayStart = NULL, $sort = NULL, $sortdir = NULL, $search, $search_arr)->toArray();
        ...
        $data = PrintJobVendor::getPrintJobVendorData($iDisplayLength, $iDisplayStart, $sort, $sortdir, $search, $search_arr)->toArray();
        $cnt = count($data);
        for ($i = 0; $i < $cnt; $i++) {

            if($data[$i]->status == PrintJobVendor::ACTIVE){
                $status =  "Active" ;
            }else if($data[$i]->status == PrintJobVendor::INACTIVE){
                $status =  "Inactive" ;
            } 
    ...
    */
    public function PostAjaxData(Request $request) {
        ...
        if (isset($post['customActionName'])) {
            ...
            $module_id = Config::get('constants.module_id.print_job_vendor');
            ...
            if ($post['customActionName'] == "Active" || $post['customActionName'] == 'Inactive') {
                ...
                if(is_array($post['id']) && !empty($post['id'])){
                    foreach($post['id'] as $id){
                        $log_text = "Print Job Vendor " . $id . " Status updated to ".$post['customActionName'];
                    ...
                    $log_text = "Print Job Vendor " . $post['id'] . " Status updated to ".$post['customActionName'];
    ...
    */
    private function validateAdd($data = array()) {
        $rules = array(
            'name' => 'required|max:150|unique:mas_book_press,name,NULL,id',
            'address' => 'required|max:300',
        );
        $messages = array(
            'name.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Print Job Vendor'),
            'name.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'Print Job Vendor'),
            'name.max' => sprintf(Config::get('messages.validation_msg.maxlength'), '150', 'Print Job Vendor'),
    ...
    */
    private function validateEdit($data = array()) {
        $rules = array(
            'name' => 'required|max:150|unique:mas_book_press,name,' . $data['id'] . ',id',
            'address' => 'required|max:300',
        );
        $messages = array(
            'name.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Print Job Vendor'),
    ...
    */            
    private function insertRecord($data = array()) {
        ...
        $insert_array = array(
            ...
            'status' => (isset($data['status']) ? $data['status'] : PrintJobVendor::INACTIVE),
        );

        $insert = PrintJobVendor::addPrintJobVendor($insert_array);
        ...
        if (!empty($insert)) {
            ...
            $module_id = Config::get('constants.module_id.print_job_vendor');
            $log_text = "Print Job Vendor " . $data['name'] . " added";
    ...
    */  
    private function updateRecord($data = array()) {
        ...
        $update = PrintJobVendor::updatePrintJobVendor($id,$update_array);
       ...
       if (isset($update)) {
            ...
            $module_id = Config::get('constants.module_id.print_job_vendor');
            $log_text = "Print Job Vendor " . $data['name'] . " edited";
    ...
    */ 
    private function updateStatus($id = array(), $status) {
        if (!empty($id)) {
            ...
            $update = PrintJobVendor::updatePrintJobVendorStatus($id, $status);
        }
        if (isset($update)) {
            return 1;
        } else {
    ...
    */
    private function deleteRecord($id = array()) {
        if (!empty($id)) {
            $printJobVendors = PrintJobVendor::getPrintJobVendorDataFromId($id);
            if(!empty($printJobVendors)){
                foreach ($printJobVendors as $data) {
                    ## Start Add Activity
                    $reference_id = (isset($data['id']) ? $data['id'] : "");
                    $module_id = Config::get('constants.module_id.print_job_vendor');
                    $log_text = "Print Job Vendor ". $data['name']. " deleted";
                    $activity_arr= array("admin_id" => Auth::guard('admin')->user()->id, "module_id" => $module_id, "reference_id" => $reference_id, "no" => '', "log_text" => $log_text, "req_data" => $data);
                    usr_addActivityLog($activity_arr);
                    ## End Add Activity
                 }                  
            $delete_var = PrintJobVendor::deletePrintJobVendorData($id);
            }
