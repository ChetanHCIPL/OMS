...
class SemesterController extends Controller {
    ...
    /**
    *  Load Semester List Page
    *
    * @param    void
    * @return   template
    */
    public function index() {
        return view('admin/master/semester_list');
    }

    /**
    * Get Semester data and pass json response to data table
    *
    * @param  array $request
    * @return json $records
    */
    public function ajaxData(Request $request) {
        ...
        $tot_records_data = Semester::getSemesterData($iDisplayLength = NULL, $iDisplayStart = NULL, $sort = NULL, $sortdir = NULL, $search, $search_arr)->toArray();
        ...
        $data = Semester::getSemesterData($iDisplayLength, $iDisplayStart, $sort, $sortdir, $search, $search_arr)->toArray();
        $cnt = count($data);
        for ($i = 0; $i < $cnt; $i++) {

            if($data[$i]->status == Semester::ACTIVE){
                $status =  "Active" ;
            }else if($data[$i]->status == Semester::INACTIVE){
                $status =  "Inactive" ;
            } 
    ...
    */
    public function PostAjaxData(Request $request) {
        ...
        if (isset($post['customActionName'])) {
            ...
            $module_id = Config::get('constants.module_id.semester');
            ...
            if ($post['customActionName'] == "Active" || $post['customActionName'] == 'Inactive') {
                ...
                if(is_array($post['id']) && !empty($post['id'])){
                    foreach($post['id'] as $id){
                        $log_text = "Semester " . $id . " Status updated to ".$post['customActionName'];
                    ...
                    $log_text = "Semester " . $post['id'] . " Status updated to ".$post['customActionName'];
    ...
    */
    private function validateAdd($data = array()) {
        $rules = array(
            'name' => 'required|max:150|unique:mas_semester,name,NULL,id',
        );
        $messages = array(
            'name.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Semester'),
            'name.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'Semester'),
            'name.max' => sprintf(Config::get('messages.validation_msg.maxlength'), '150', 'Semester'),
        );
    ...
    */
    private function validateEdit($data = array()) {
        $rules = array(
            'name' => 'required|max:150|unique:mas_semester,name,' . $data['id'] . ',id',
        );
        $messages = array(
            'name.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'Semester'),
            'name.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'Semester'),
    ...
    /**
    * Function : Insert Record of Semesters 
    *
    * @param  array $data
    * @return $result
    */            
    private function insertRecord($data = array()) {
        $insert_array = array(
            ...
            'status' => (isset($data['status']) ? $data['status'] : Semester::INACTIVE),
        );

        $insert = Semester::addSemester($insert_array);
        if (!empty($insert)) {
            $result = 1;
            ...
            $module_id = Config::get('constants.module_id.semester');
            $log_text = "Semester " . $data['name'] . " added";
    ...
    /**
    * Function : Update Record of Semester
    *
    * @param  array $data
    * @return $result;
    */  
    private function updateRecord($data = array()) {
        ...
        $update = Semester::updateSemester($id,$update_array);
       ...
       if (isset($update)) {
            ...
            $module_id = Config::get('constants.module_id.semester');
            $log_text = "Semester " . $data['name'] . " edited";
    ...
    */ 
    private function updateStatus($id = array(), $status) {
        if (!empty($id)) {
            ...
            $update = Semester::updateSemesterStatus($id, $status);
        }
        if (isset($update)) {
            return 1;
        } else {
     ...
     /** 
    * Function : Delete Records of  Semester
    * @param  array $id
    * @return boolean
    */
    private function deleteRecord($id = array()) {
        if (!empty($id)) {
            $semesters = Semester::getSemesterDataFromId($id);
            if(!empty($semesters)){
                foreach ($semesters as $data) {
                    ## Start Add Activity
                    $reference_id = (isset($data['id']) ? $data['id'] : "");
                    $module_id = Config::get('constants.module_id.semester');
                    $log_text = "Semester ". $data['name']. " deleted";
                    $activity_arr= array("admin_id" => Auth::guard('admin')->user()->id, "module_id" => $module_id, "reference_id" => $reference_id, "no" => '', "log_text" => $log_text, "req_data" => $data);
                    usr_addActivityLog($activity_arr);
                    ## End Add Activity
                 }                  
            $delete_var = Semester::deleteSemesterData($id);
            }
