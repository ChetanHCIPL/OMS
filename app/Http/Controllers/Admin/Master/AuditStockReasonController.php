...
use App\Models\AuditStockReason;

class AuditStockReasonController extends Controller {
    ...
    /**
    *  Load AuditStockReason List Page
    *
    * @param    void
    * @return   template
    */
    public function index() {
        return view('admin/master/auditstockreason_list');
    }

    /**
    * Get AuditStockReason data and pass json response to data table
    *
    * @param  array $request
    * @return json $records
    */
    public function ajaxData(Request $request) {
        ...
        $tot_records_data = AuditStockReason::getAuditStockReasonData($iDisplayLength = NULL, $iDisplayStart = NULL, $sort = NULL, $sortdir = NULL, $search, $search_arr)->toArray();
        ...
        $data = AuditStockReason::getAuditStockReasonData($iDisplayLength, $iDisplayStart, $sort, $sortdir, $search, $search_arr)->toArray();
        $cnt = count($data);
        for ($i = 0; $i < $cnt; $i++) {

            if($data[$i]->status == AuditStockReason::ACTIVE){
                $status =  "Active" ;
            }else if($data[$i]->status == AuditStockReason::INACTIVE){
                $status =  "Inactive" ;
            } 
            ...
            $edit = '---';
            $view = '';
            if (per_hasModuleAccess('AuditStockReason', 'Edit')) {
            ...
            $records["data"][] = array(
                ...
                '<span id="gfname' . $data[$i]->id . '"> <span id="gname' . $data[$i]->id . '" class="d-none">' . $data[$i]->title . '</span>'.$data[$i]->title  . '</span>',
    ...
    */
    public function PostAjaxData(Request $request) {
        ...
        if (isset($post['customActionName'])) {
            ...
            $module_id = Config::get('constants.module_id.auditstockreason');
            ...
            if ($post['customActionName'] == "Active" || $post['customActionName'] == 'Inactive') {
                ...
                if(is_array($post['id']) && !empty($post['id'])){
                    foreach($post['id'] as $id){
                        $log_text = "AuditStockReason " . $id . " Status updated to ".$post['customActionName'];
                    ...
                    $log_text = "AuditStockReason " . $post['id'] . " Status updated to ".$post['customActionName'];
    ...
    */
    private function validateAdd($data = array()) {
        $rules = array(
            'title' => 'required|max:150|unique:mas_audit_stock_reason,title,NULL,id',
        );
        $messages = array(
            'title.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'AuditStockReason'),
            'title.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'AuditStockReason'),
        );
    ...
    */
    private function validateEdit($data = array()) {
        $rules = array(
            'title' => 'required|max:150|unique:mas_audit_stock_reason,title,' . $data['id'] . ',id',
        );
        $messages = array(
            'title.required' => sprintf(Config::get('messages.validation_msg.required_field'), 'AuditStockReason'),
            'title.unique' => sprintf(Config::get('messages.validation_msg.unique_field'), 'AuditStockReason'),
    ...
    /**
    * Function : Insert Record of AuditStockReason 
    *
    * @param  array $data
    * @return $result
    */            
    private function insertRecord($data = array()) {
        $insert_array = array(
            ...
            'status' => (isset($data['status']) ? $data['status'] : AuditStockReason::INACTIVE),
        );

        $insert = AuditStockReason::addAuditStockReason($insert_array);
        if (!empty($insert)) {
            $result = 1;
            ...
            $module_id = Config::get('constants.module_id.auditstockreason');
            $log_text = "AuditStockReason " . $data['title'] . " added";
    ...
    /**
    * Function : Update Record of AuditStockReason
    *
    * @param  array $data
    * @return $result;
    */  
    private function updateRecord($data = array()) {
        ...
        $update = AuditStockReason::updateAuditStockReason($id,$update_array);
       ...
       if (isset($update)) {
            ...
            $module_id = Config::get('constants.module_id.auditstockreason');
            $log_text = "AuditStockReason " . $data['title'] . " edited";
    ...
    */ 
    private function updateStatus($id = array(), $status) {
        if (!empty($id)) {
            ...
            $update = AuditStockReason::updateAuditStockReasonStatus($id, $status);
        }
        if (isset($update)) {
            return 1;
        } else {
     ...
     /** 
    * Function : Delete Records of  AuditStockReason
    * @param  array $id
    * @return boolean
    */
    private function deleteRecord($id = array()) {
        if (!empty($id)) {
            $auditstockreason = AuditStockReason::getAuditStockReasonDataFromId($id);
            if(!empty($auditstockreason)){
                foreach ($auditstockreason as $data) {
                    ## Start Add Activity
                    $reference_id = (isset($data['id']) ? $data['id'] : "");
                    $module_id = Config::get('constants.module_id.auditstockreason');
                    $log_text = "AuditStockReason ". $data['title']. " deleted";
                    $activity_arr= array("admin_id" => Auth::guard('admin')->user()->id, "module_id" => $module_id, "reference_id" => $reference_id, "no" => '', "log_text" => $log_text, "req_data" => $data);
                    usr_addActivityLog($activity_arr);
                    ## End Add Activity
                 }                  
            $delete_var = AuditStockReason::deleteAuditStockReasonData($id);
            }
